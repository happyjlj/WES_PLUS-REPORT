...
try:
	tree = ET.parse("family_template.xml",parser=ET.XMLParser(encoding='utf-8'))		 #打开xml文档
	root = tree.getroot()				 #获得root节点
except Exception, e:
	print "Error:cannot parse file:family_template.xml."+str(e)
	sys.exit(1)
...
def get_extend_report(root_node):
	...
	all_table_data=[]
	table_index=[]
	for sample in extend_node.findall("sample"):
		table_index.append(sample.attrib['bus_code'].strip())
		result = []
		for item in sample.findall("item"):
			site=[]
			for child in item:
				site.append(child.text.strip())
			result.append(site)
		all_table_data.append(result)
	return all_table_data,table_index

def get_verify_result(root_node,config_path=os.path.split(os.path.realpath(__file__))[0]):
