...
def buildNewsXmlFile(file):
	...
	tree.write("family_template.xml", 'utf8')
...
###############创建扩展报告，ｌａｂｅｌ为添加的属性的key################
def createExtendReport(node,bus_code_list,data,label):
	if len(data):
		index=0 
		for sample in data:
			sample_xml=ET.SubElement(node,"sample")
			sample_xml.set(label,bus_code_list[index])
			index+=1
			if len(sample):
				for item in sample:
					if len(item)==12:
						item_xml=ET.SubElement(sample_xml, "item")
...
def read_family_info(info_file):
	...
	#label,extend=extract_extend_or_verify(content,default="extend")
...
###########提取扩展报告和验证数据中的内容##########
def extract_extend_or_verify(content,default="extend",char_num=6):
	#------读取扩展报告中的内容-------#
	if default=="extend":
		#pattern=r'<(?P<bus_code>\w{%s})>(?P<item>.*?)</\w{%s}>'%(char_num,char_num)  
		pattern=r'<(?P<bus_code>\w+\d+)>(?P<item>.*?)</\w+\d+>'
		...
		if match:
			for item in match:
				#print item
				if len(item)>1:
					bus_code_list.append(item[0].strip())
					all_sites.append(deal_extend_list(item[1]))
		#print bus_code_list
		#sys.exit(1)
		result=bus_code_list,all_sites
	#-------读取验证数据内容------------#
	elif default=="verify":
